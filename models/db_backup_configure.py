...
class DbBackupConfigure(models.Model):
    ...
    def get_onedrive_tokens(self, authorize_code):
        ...
        token_url = "https://login.microsoftonline.com/common/oauth2/v2.0/token"
        try:
            res = requests.post(token_url, data=data, headers=headers, timeout=60)
            res.raise_for_status()
            response = res.json() if res.ok else {}
            if response:
                expires_in = response.get('expires_in', 0)
    ...
    def generate_gdrive_refresh_token(self):
        ...
        try:
            res = requests.post(GOOGLE_TOKEN_ENDPOINT, data=data, headers=headers, timeout=60)
            res.raise_for_status()
            response = res.json() if res.ok else {}
            if response:
                expires_in = response.get('expires_in', 0)
            ...
            # Google Drive backup
                ...
                try:
                    ...
                    try:
                        ...
                        _logger.info('Starting Google Drive upload for file: %s, size: %s bytes', backup_filename, file_size)
                        ...
                        if not upload_url:
                            raise ValueError("Failed to get upload URL from Google Drive")
                        
                        _logger.info('Google Drive upload session created successfully')
                        ...
                        with open(temp.name, 'rb') as f:
                            while uploaded < file_size:
                                ...
                                chunk_end = uploaded + chunk_length - 1
                                ...
                                progress_percent = (uploaded / file_size) * 100
                                _logger.info('Google Drive upload progress: %.2f%% (%s/%s bytes)', 
                                           progress_percent, uploaded, file_size)
                                
                                # Check if upload is complete
                                if upload_response.status_code in [200, 201]:
                                    _logger.info('Google Drive upload completed successfully')
                                    break
                                elif upload_response.status_code not in [308]:  # 308 = resume incomplete
                                    upload_response.raise_for_status()
                        
                        # Remove old backups if configured
                        if rec.auto_remove:
                            ...
                            for file in files_req.json().get('files', []):
                                ...
                                file_date_req.raise_for_status()
                                ...
                                if diff_days >= rec.days_to_remove:
                                    ...
                                    _logger.info('Removed old backup file: %s', file.get('name'))
                        ...
                        _logger.error('Google Drive Exception: %s', e, exc_info=True)
                        if rec.notify_user:
                        ...
                        try:
                            ...
                            _logger.warning('Failed to cleanup temp file: %s', cleanup_error)
                            
                except Exception as outer_error:
                    rec.generated_exception = str(outer_error)
                    _logger.error('Google Drive Backup Failed: %s', outer_error, exc_info=True)
                    if rec.notify_user:
                        mail_template_failed.send_mail(rec.id, force_send=True)
                    # Don't raise ValidationError here, just log the error
                    # This allows other backups to continue
            # Dropbox backup
